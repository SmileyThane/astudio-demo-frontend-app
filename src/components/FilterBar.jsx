...
const FilterBar = ({tableProps, type}) => {
    const {
        setFilter,
        pageSize,
        setPageSize,
    ...
    return (
        <div className="filter-bar ">
            ...
            <select value={filterCategory} onChange={handleFilterCategory}>
                ...
                {Object.keys(tableProps).map((item, rowIndex) => (
                    <option value={Object.values(tableProps)[rowIndex]}>{item}</option>
                ))}
            </select>

            {filterCategory && type === 'users' &&
                <input
                    type="text"
                    value={externalFilter}
                    onChange={handleExternalFilter}
